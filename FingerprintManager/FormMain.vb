...
Public Class FormMain
    ...
    Private emptyBitmap As New Bitmap(1, 1)

    Private featureExtractor As JYFeatureExtractor
    Private featureDisplay1 As MTripletsDisplay

    Private features As JYFeatures

    Private Sub FormMain_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        If selectedScanner IsNot Nothing Then selectedScanner.Scanner.SetDiodesStatus(DiodesStatus.turn_off, DiodesStatus.turn_off)
    ...
    Private Sub FormMain_KeyUp(sender As Object, e As KeyEventArgs) Handles Me.KeyUp
        If e.KeyCode = Keys.Space Then
            ...
            Using g As Graphics = Graphics.FromImage(img)
                featureDisplay1.Show(featureExtractor.MtiaExtractor.ExtractFeatures(img), g)
            End Using

            PictureBoxFeatures.Image = img
    ...
    Private Sub FormMain_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        scannerThread = New Thread(AddressOf ScannerLoop)
        scannerThread.IsBackground = True
        scannerThread.Start()

        featureExtractor = New JYFeatureExtractor()
        featureExtractor.MtiaExtractor = New Ratha1995MinutiaeExtractor()
        featureExtractor.SkeletonImgExtractor = New Ratha1995SkeImgExtractor()
    ...
    Private Sub ScannerLoop()
        Do
            If selectedScanner Is Nothing OrElse selectedScanner.Scanner Is Nothing Then
                ...
                Thread.Sleep(250)
            Else
                If selectedScanner.Scanner.IsFingerPresent Or True Then
            ...
            Me.Invoke(New MethodInvoker(Sub()
                                            LabelInfo.Visible = Not PictureBoxFingerprint.Image.Equals(emptyBitmap)
                                        End Sub))
    ...
    Private Sub UpdateFrame()
        ...
        If frame IsNot Nothing AndAlso frame.Length > 0 Then
            Using g As Graphics = PictureBoxFingerprint.CreateGraphics()
                Dim bmp As Bitmap = ResizeImage(BitmapAPI.GetBitmap(g.GetHdc(), frame), selectedScanner.Scanner.ImageSize.Width \ 2, selectedScanner.Scanner.ImageSize.Height \ 2)
    ...
    Private Sub ButtonSave_Click(sender As Object, e As EventArgs) Handles ButtonSave.Click
        Dim fileName As String = IO.Path.Combine(My.Application.Info.DirectoryPath, TextBoxName.Text + ".fpd")
        If IO.File.Exists(fileName) Then IO.File.Delete(fileName)
        BinarySerializer.Serialize(features, fileName)

        PictureBoxFeatures.Image = Nothing
        TextBoxName.Text = ""
        SetCtrlsState(False)
    End Sub
End Class
